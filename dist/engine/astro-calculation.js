...
function calculateBaseValues(monthTh, yearBe, day, hour, minute) {
    ...
    const solarAdjF6 = yearRelative1181 * 0.25875 +
...
function getQuadrantVals(value) {
    ...
    let quadrantVal;
    if (quadrant === 1)
        quadrantVal = wrappedInput;
    else if (quadrant === 2)
        quadrantVal = 10800 - wrappedInput;
    else if (quadrant === 3)
        quadrantVal = wrappedInput - 10800;
    else
        quadrantVal = 21600 - wrappedInput;
    return [quadrant, quadrantVal, signMultiplier];
}
function getTableAdj(quadrantVal) {
...
function getSecondaryAdjParams(value) {
    const wrappedInput = wrap21600(value);
    const quadrant = Math.floor(wrappedInput / 5400) + 1;
    let secondaryVal;
    if (quadrant === 1)
        secondaryVal = 5400 - wrappedInput;
    else if (quadrant === 2)
        secondaryVal = wrappedInput - 5400;
    else if (quadrant === 3)
        secondaryVal = 16200 - wrappedInput;
    else
        secondaryVal = wrappedInput - 16200;
...
function calculateSunPrecisePosition(monthTh, yearBe, day, hour, minute) {
    ...
    let quadrantValB28;
    if (quadrant === 1)
        quadrantValB28 = anomalyBaseB26;
    else if (quadrant === 2)
        quadrantValB28 = 10800 - anomalyBaseB26;
    else if (quadrant === 3)
        quadrantValB28 = anomalyBaseB26 - 10800;
    else
        quadrantValB28 = 21600 - anomalyBaseB26;
    const e27Val = Math.floor(Math.trunc(quadrantValB28 / 900));
    const e28Val = e27Val + 1;
    const gE27 = (_a = G_VALS_SUN[e27Val]) !== null && _a !== void 0 ? _a : G_VALS_SUN[6];
    const gE28 = (_b = G_VALS_SUN[e28Val]) !== null && _b !== void 0 ? _b : G_VALS_SUN[6];
...
function calculateMoon(monthTh, yearBe, day, hour, minute) {
    ...
    const [relativeJulianDay, timeFraction, , , solarPositionMean] = calculateBaseValues(monthTh, yearBe, day, hour, minute);
    ...
    let quadrantValB39;
    if (b38Val === 1)
        quadrantValB39 = b37Val;
    else if (b38Val === 2)
        quadrantValB39 = 10800 - b37Val;
    else if (b38Val === 3)
        quadrantValB39 = b37Val - 10800;
    else
        quadrantValB39 = 21600 - b37Val;
    const e38Val = Math.floor(Math.trunc(quadrantValB39 / 900));
    const e39Val = e38Val + 1;
    const gE38 = (_a = G_VALS_MOON[e38Val]) !== null && _a !== void 0 ? _a : G_VALS_MOON[6];
    const gE39 = (_b = G_VALS_MOON[e39Val]) !== null && _b !== void 0 ? _b : G_VALS_MOON[6];
...
function calculateAscendant(day, monthTh, yearBe, hour, minute, province) {
    ...
    const provinceOffsetMin = (_a = PROVINCE_TIME_OFFSETS[province]) !== null && _a !== void 0 ? _a : 18;
    const localTimeMin = hour * 60 + minute;
    const cumulativeDurationAtSunSignStart = SIGN_DURATIONS_MINUTES.slice(0, sunSignIndex).reduce((sum, duration) => sum + duration, 0);
    ...
    if (ascendantTimeMin < 0)
        ascendantTimeMin += 1440;
    ...
    for (let i = 0; i < SIGN_DURATIONS_MINUTES.length; i++) {
        ...
        if (fallsInSign) {
            ...
            if (timeIntoAscendantSign < 0)
                timeIntoAscendantSign += 1440;
